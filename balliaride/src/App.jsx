...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .input-dark { background: #1A1A1A; border: 1px solid #2A2A2A; border-radius: 12px; color: #FFF; font-family: 'DM Sans', sans-serif; outline: none; transition: border 0.2s; width: 100%; }
  .input-dark:focus { border-color: #FFCC00; }
  ...
  .tab-active { background: #FFCC00; color: #0A0A0A; }
  ...
  @media(max-width:768px){ .hide-mobile{display:none!important} .stack-mobile{flex-direction:column!important} }
`;
...
const BikeIllustration = () => (
  <svg viewBox="0 0 200 120" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",maxWidth:200}}>
    ...
    <path d="M60 75 L50 90" stroke="#888" strokeWidth="2"/>
    <path d="M140 75 L150 90" stroke="#888" strokeWidth="2"/>
  </svg>
);
...
const RikshawIllustration = () => (
  <svg viewBox="0 0 220 140" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",maxWidth:220}}>
    ...
    <path d="M175 75 L200 65 L195 90 L175 90Z" fill="#FFCC00" fillOpacity="0.6"/>
    <text x="110" y="65" textAnchor="middle" fill="#FFCC00" fontSize="11" fontWeight="700">E-RIKSHAW</text>
    <path d="M20 80 L5 75 L5 95 L20 95" stroke="#FFCC00" strokeWidth="2" fill="none"/>
  </svg>
);
...
const CabIllustration = () => (
  <svg viewBox="0 0 240 130" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",maxWidth:240}}>
    ...
    <rect x="15" y="62" width="30" height="18" rx="4" fill="#FFCC00" fillOpacity="0.8"/>
    <rect x="195" y="62" width="25" height="10" rx="3" fill="#FF4444" fillOpacity="0.8"/>
    <path d="M195 72 L220 72" stroke="#FF8888" strokeWidth="2"/>
...
const Spinner = ({ text = "Finding your ride..." }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 24, padding: 40 }}>
    <div style={{ position: "relative", width: 80, height: 80 }}>
      ...
      <div style={{ position: "absolute", inset: "50%", transform: "translate(-50%,-50%)", width: 12, height: 12, background: "#FFCC00", borderRadius: "50%" }} className="animate-pulse"/>
    </div>
  ...
  return (
    <div className="card-dark" style={{ padding: "20px 24px", marginBottom: 12 }}>
      <button onClick={() => setOpen(!open)} style={{ background: "none", border: "none", color: "#FFF", width: "100%", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 15, fontWeight: 500 }}>
        {q}
        <Icon name="chevronDown" size={18} style={{ transform: open ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.3s", color: "#FFCC00" }}/>
      </button>
...
export default function Balliaride() {
  const [page, setPage] = useState("home"); // home | admin | driver | profile
  ...
  const [bookingStep, setBookingStep] = useState(1); // 1=pickup/drop, 2=select service, 3=confirm, 4=searching, 5=booked
  ...
  const statusColor = (s) => s === "Completed" ? "green" : s === "Active" ? "yellow" : s === "Pending" ? "#888" : "red";

  // ─── NAVBAR ─────────────────────────────────────────────────────────────
  const Navbar = () => (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "0 24px", height: 70, display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(10,10,10,0.9)", borderBottom: "1px solid #1A1A1A", backdropFilter: "blur(16px)" }}>
      ...
      <div className="hide-mobile" style={{ display: "flex", gap: 12, alignItems: "center" }}>
        {user ? (
          <>
            ...
            <button className="btn-outline" style={{ padding: "8px 18px", fontSize: 14, borderColor: "#333", color: "#888" }} onClick={() => { setUser(null); setPage("home"); }}>Logout</button>
          </>
        ) : (
          <>
        ...
        <button className="btn-outline" style={{ padding: "8px 14px", fontSize: 12, borderColor: "#333", color: "#888" }} onClick={() => setPage("admin")}>Admin</button>
        <button className="btn-outline" style={{ padding: "8px 14px", fontSize: 12, borderColor: "#333", color: "#888" }} onClick={() => setPage("driver")}>Driver</button>
      </div>
      <button onClick={() => setMobileMenu(!mobileMenu)} style={{ background: "none", border: "none", color: "#FFF", cursor: "pointer", display: "none" }} className="show-mobile">
  ...
  // ─── AUTH MODAL ──────────────────────────────────────────────────────────
  const AuthModal = () => (
    <div className="overlay" onClick={() => setShowAuth(null)}>
      <div className="modal fade-in" onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          <button onClick={() => setShowAuth(null)} style={{ background: "#2A2A2A", border: "none", borderRadius: 10, width: 36, height: 36, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#888" }}>
        ...
        {authStep === 1 ? (
          <>
            {showAuth === "signup" && (
              <div style={{ marginBottom: 16 }}>
                ...
                <input className="input-dark" placeholder="Enter your name" value={name} onChange={e => setName(e.target.value)} style={{ padding: "12px 16px", fontSize: 15 }}/>
              </div>
            )}
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 13, color: "#888", marginBottom: 8, display: "block" }}>Mobile Number</label>
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ background: "#2A2A2A", border: "1px solid #333", borderRadius: 12, padding: "12px 14px", color: "#888", fontSize: 15, minWidth: 60 }}>+91</div>
                <input className="input-dark" placeholder="10-digit mobile" value={phone} onChange={e => setPhone(e.target.value)} maxLength={10} style={{ padding: "12px 16px", fontSize: 15 }}/>
              </div>
            </div>
            <button className="btn-yellow" style={{ width: "100%", padding: "14px", fontSize: 15 }} onClick={handleSendOtp}>
              Send OTP →
            </button>
          ...
          <>
            <p style={{ color: "#888", fontSize: 14, marginBottom: 24 }}>Enter the 6-digit OTP sent to +91 {phone}</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 28 }}>
              {otp.map((v, i) => (
                <input key={i} id={`otp-${i}`} maxLength={1} value={v} onChange={e => {
                  const newOtp = [...otp];
    ...
    <div className="overlay">
      <div className="modal fade-in" style={{ maxWidth: 560 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
          ...
          {bookingStep < 4 && (
            <button onClick={() => { setBookingModal(false); setBookingStep(1); setSelectedService(null); }} style={{ background: "#2A2A2A", border: "none", borderRadius: 10, width: 36, height: 36, cursor: "pointer", color: "#888" }}>
              <Icon name="close" size={16}/>
            </button>
          )}
        </div>
        ...
        {bookingStep === 1 && (
          <>
            <div style={{ position: "relative", marginBottom: 12 }}>
              <div style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#22c55e" }}><Icon name="mapPin" size={18}/></div>
              <input className="input-dark" placeholder="Pickup location (e.g. Civil Lines)" value={pickup} onChange={e => setPickup(e.target.value)} style={{ padding: "14px 14px 14px 42px", fontSize: 15 }}/>
            </div>
            <div style={{ position: "relative", marginBottom: 28 }}>
              <div style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#FFCC00" }}><Icon name="mapPin" size={18}/></div>
              <input className="input-dark" placeholder="Drop location (e.g. Railway Station)" value={drop} onChange={e => setDrop(e.target.value)} style={{ padding: "14px 14px 14px 42px", fontSize: 15 }}/>
            </div>
            <div style={{ background: "#111", borderRadius: 12, height: 160, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24, border: "1px solid #222", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 30% 50%, rgba(255,204,0,0.08) 0%, transparent 60%)" }}/>
              <div style={{ textAlign: "center", color: "#555" }}>
                <Icon name="mapPin" size={32} style={{ color: "#FFCC00", margin: "0 auto 8px" }}/>
                <p style={{ fontSize: 13 }}>Map preview — Ballia, UP</p>
              </div>
            </div>
            <button className="btn-yellow" style={{ width: "100%", padding: 14, fontSize: 15 }} onClick={() => pickup && drop ? setBookingStep(2) : alert("Please enter pickup and drop location")}>
              Find Rides →
            </button>
          </>
        )}
        ...
        {bookingStep === 2 && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {services.map(s => (
                <div key={s.id} onClick={() => setSelectedService(s)} style={{ padding: 20, borderRadius: 16, border: `2px solid ${selectedService?.id === s.id ? "#FFCC00" : "#2A2A2A"}`, background: selectedService?.id === s.id ? "rgba(255,204,0,0.06)" : "#1A1A1A", cursor: "pointer", display: "flex", alignItems: "center", gap: 20, transition: "all 0.2s" }}>
                  <div style={{ width: 90 }}>{s.icon}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 4 }}>{s.label}</div>
          ...
          <>
            <div style={{ background: "#111", borderRadius: 16, padding: 20, marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", paddingBottom: 16, borderBottom: "1px solid #222", marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ fontWeight: 600, fontSize: 15 }}>{pickup}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>TO</div>
                  <div style={{ fontWeight: 600, fontSize: 15 }}>{drop}</div>
                </div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>SERVICE</div>
                  <div style={{ fontWeight: 600 }}>{selectedService?.label}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>EST. FARE</div>
                  <div style={{ color: "#FFCC00", fontWeight: 700, fontFamily: "'Sora',sans-serif", fontSize: 18 }}>{selectedService?.fare}</div>
                </div>
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              ...
              {["Cash","UPI","Wallet"].map(pm => (
                <div key={pm} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, border: "1px solid #2A2A2A", marginBottom: 8, cursor: "pointer" }}>
                  <Icon name={pm === "Wallet" ? "wallet" : "check"} size={18} style={{ color: "#FFCC00" }}/>
        ...
        {bookingStep === 5 && (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            <div style={{ background: "#111", borderRadius: 12, padding: 16, marginBottom: 24 }}>
              <div style={{ color: "#888", fontSize: 12, marginBottom: 4 }}>BOOKING ID</div>
              <div className="sora" style={{ color: "#FFCC00", fontSize: 20, fontWeight: 800 }}>{bookingId}</div>
            </div>
            <div style={{ background: "#1A1A1A", borderRadius: 16, padding: 16, textAlign: "left", marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <button style={{ marginLeft: "auto", background: "#FFCC00", border: "none", borderRadius: 10, padding: "8px 14px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Call</button>
              </div>
            </div>
            <button className="btn-yellow" style={{ width: "100%", padding: 14, fontSize: 15 }} onClick={() => { setBookingModal(false); setBookingStep(1); setSelectedService(null); setPickup(""); setDrop(""); }}>
              Done
            </button>
          </div>
        )}
  ...
  const HomePage = () => (
    <div style={{ paddingTop: 70 }}>
      ...
      {/* Hero */}
      <section style={{ minHeight: "92vh", display: "flex", alignItems: "center", padding: "60px 24px", maxWidth: 1200, margin: "0 auto", gap: 60 }} className="stack-mobile">
        {/* LEFT */}
        <div style={{ flex: 1 }} className="fade-in">
          ...
          <div className="glass" style={{ borderRadius: 20, padding: 24, maxWidth: 480 }}>
            <div style={{ position: "relative", marginBottom: 14 }}>
              <div style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#22c55e" }}><Icon name="mapPin" size={18}/></div>
              <input className="input-dark" placeholder="Pickup location in Ballia" value={pickup} onChange={e => setPickup(e.target.value)} style={{ padding: "14px 14px 14px 42px", fontSize: 15 }}/>
            </div>
            <div style={{ position: "relative", marginBottom: 20 }}>
              <div style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#FFCC00" }}><Icon name="mapPin" size={18}/></div>
              <input className="input-dark" placeholder="Drop location" value={drop} onChange={e => setDrop(e.target.value)} style={{ padding: "14px 14px 14px 42px", fontSize: 15 }}/>
            </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 24, alignItems: "center" }}>
          <div className="animate-float" style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 20, padding: "20px 30px", textAlign: "center" }}>
            <BikeIllustration/>
            <div className="sora" style={{ color: "#FFCC00", fontWeight: 700, marginTop: 8 }}>Bike Taxi</div>
          </div>
          <div style={{ display: "flex", gap: 16 }}>
            <div className="animate-float2" style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 20, padding: "16px 20px", textAlign: "center", flex: 1 }}>
              <RikshawIllustration/>
              <div className="sora" style={{ color: "#FFCC00", fontWeight: 700, marginTop: 6, fontSize: 13 }}>E-Rikshaw</div>
            </div>
            <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 20, padding: "16px 20px", textAlign: "center", flex: 1, animation: "float 6s ease-in-out infinite" }}>
              <CabIllustration/>
      ...
      <section style={{ padding: "80px 24px", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>Rides for every need</h2>
          </div>
            ...
            {services.map(s => (
              <div key={s.id} className="card-dark" style={{ padding: 28, textAlign: "center" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20, background: "#111", borderRadius: 10, padding: "10px 16px" }}>
                  <span style={{ color: "#888", fontSize: 13 }}>Est. Fare: <strong style={{ color: "#FFCC00" }}>{s.fare}</strong></span>
      ...
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>What we offer</h2>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>What our riders say</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 24 }}>
            {reviews.map(r => (
              <div key={r.name} className="card-dark" style={{ padding: 28 }}>
                <div style={{ display: "flex", gap: 2, marginBottom: 16 }}>
                  {[...Array(r.rating)].map((_, i) => <Icon key={i} name="star" size={16} style={{ color: "#FFCC00" }}/>)}
                </div>
      ...
      {/* Download App */}
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", background: "linear-gradient(135deg, #1A1500 0%, #1A1A1A 100%)", border: "1px solid #FFCC00", borderRadius: 28, padding: "60px 40px", textAlign: "center", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: -60, right: -60, width: 200, height: 200, background: "rgba(255,204,0,0.05)", borderRadius: "50%" }}/>
          <span className="section-tag">Get the App</span>
          <h2 className="sora" style={{ fontSize: 40, fontWeight: 800, marginBottom: 12 }}>Download Balliaride</h2>
      ...
      <section style={{ padding: "80px 24px", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>Your safety, our priority</h2>
          </div>
      ...
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", gap: 48, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 260 }}>
            ...
            <h2 className="sora" style={{ fontSize: 40, fontWeight: 800, marginBottom: 16 }}>Become a driver partner</h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
              {["Weekly payouts directly to your account","Flexible hours — drive when you want","Full support & training provided"].map(b => (
                <div key={b} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 22, height: 22, background: "rgba(255,204,0,0.15)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <button className="btn-yellow" style={{ padding: "14px 28px", fontSize: 15 }} onClick={() => setPage("driver")}>
              Register as Driver →
            </button>
          </div>
          <div style={{ flex: 1, minWidth: 260, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {[["₹18k+","Avg Monthly Earning"],["500+","Active Drivers"],["4.8★","Avg Rating"],["24/7","Support"]].map(([n, l]) => (
              <div key={l} style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 20, textAlign: "center" }}>
                <div className="sora" style={{ fontSize: 26, fontWeight: 800, color: "#FFCC00", marginBottom: 4 }}>{n}</div>
      ...
      <section style={{ padding: "80px 24px", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>Common questions</h2>
          </div>
          {[
            { q: "How do I book a ride on Balliaride?", a: "Simply enter your pickup and drop location in the form on the homepage, click 'Book Ride', choose your service type (Bike, E-Rikshaw, or Cab), confirm your booking, and your driver will arrive shortly." },
            { q: "What payment methods are accepted?", a: "We accept Cash, UPI payments (Google Pay, PhonePe, Paytm), and our in-app Wallet. You can choose your preferred method at the time of booking." },
            { q: "How are fares calculated?", a: "Fares are calculated based on distance, service type, and time. Bike Taxi starts from ₹15, E-Rikshaw from ₹25, and Cab from ₹80. You'll see the estimated fare before confirming." },
            { q: "Is Balliaride available 24/7?", a: "Yes! Balliaride operates 24 hours a day, 7 days a week across Ballia and nearby areas. Driver availability may vary late at night." },
            { q: "How do I become a driver partner?", a: "Click on 'Register as Driver', fill in your details, upload your documents (DL, vehicle RC, Aadhaar), and our team will verify and onboard you within 48 hours." },
      ...
      <section style={{ padding: "80px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 className="sora" style={{ fontSize: 42, fontWeight: 800 }}>Latest from Balliaride</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 24 }}>
            {[
              ...
              <div key={b.title} className="card-dark" style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ height: 140, background: "linear-gradient(135deg, #1A1200, #2A2A1A)", display: "flex", alignItems: "center", justifyContent: "center", borderBottom: "1px solid #2A2A2A" }}>
                  <span className="badge badge-yellow" style={{ fontSize: 13 }}>{b.tag}</span>
                </div>
                <div style={{ padding: 24 }}>
                  <div style={{ color: "#555", fontSize: 12, marginBottom: 8 }}>{b.date}</div>
                  <h4 className="sora" style={{ fontSize: 17, fontWeight: 700, marginBottom: 10, lineHeight: 1.4 }}>{b.title}</h4>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 40, marginBottom: 48 }}>
            <div>
              ...
              <p style={{ color: "#555", fontSize: 13, lineHeight: 1.7 }}>Ballia's most trusted ride-hailing platform serving the city and surrounding areas.</p>
            </div>
            <div>
              <h5 className="sora" style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, color: "#FFCC00" }}>Company</h5>
              {["Home","About Us","Careers","Safety","Blog","Press"].map(l => (
                <div key={l} style={{ color: "#555", fontSize: 13, marginBottom: 10, cursor: "pointer" }} className="nav-link">{l}</div>
              ))}
            </div>
            <div>
              <h5 className="sora" style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, color: "#FFCC00" }}>Legal</h5>
              {["Privacy Policy","Customer Terms – Bike Taxi","Customer Terms – Cabs and Auto","Corporate Affairs","Captain Terms – Bike Taxi","Captain Terms – Cabs and Auto"].map(l => (
                <div key={l} style={{ color: "#555", fontSize: 13, marginBottom: 10, cursor: "pointer" }} className="nav-link">{l}</div>
              ))}
            </div>
            <div>
              <h5 className="sora" style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, color: "#FFCC00" }}>Contact</h5>
              <p style={{ color: "#555", fontSize: 13, lineHeight: 1.8 }}>Ballia, Uttar Pradesh, India 277001<br/>support@balliaride.in<br/>+91 98765 43210</p>
            </div>
          </div>
  ...
  // ─── ADMIN PANEL ─────────────────────────────────────────────────────────
  const AdminPanel = () => {
    const tabs = ["dashboard","bookings","drivers","users","analytics","blogs","complaints"];
    const stats = [
    ...
    return (
      <div style={{ display: "flex", minHeight: "100vh", paddingTop: 70 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            {tabs.map(t => (
              <div key={t} className={`sidebar-item ${adminTab === t ? "active" : ""}`} onClick={() => setAdminTab(t)}>
                <Icon name={t === "dashboard" ? "chart" : t === "bookings" ? "car" : t === "drivers" ? "users" : t === "analytics" ? "chart" : t === "blogs" ? "edit" : t === "complaints" ? "phone" : "users"} size={18}/>
        ...
        <div style={{ flex: 1, marginLeft: 220, padding: "28px 24px" }} className="admin-content">
          <style>{`.admin-content{margin-left:220px}@media(max-width:768px){.admin-content{margin-left:0!important}}`}</style>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 28 }}>
            {stats.map(s => (
              <div key={s.label} style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 20 }}>
                ...
                <div className="sora" style={{ fontSize: 28, fontWeight: 800, marginTop: 12 }}>{s.value}</div>
          ...
          {/* Mini Bar Chart */}
          <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 24, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 12, height: 120 }}>
              {[65,88,72,95,110,84,247].map((v, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  <div style={{ width: "100%", height: `${(v/247)*100}%`, background: i === 6 ? "#FFCC00" : "rgba(255,204,0,0.3)", borderRadius: "6px 6px 0 0", minHeight: 8, transition: "height 0.5s" }}/>
                  <span style={{ color: "#555", fontSize: 10 }}>{["M","T","W","T","F","S","Su"][i]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Bookings Table */}
          <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 0, overflow: "hidden" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid #2A2A2A" }}>
                    {["Booking ID","User","From → To","Service","Driver","Fare","Status","Time"].map(h => (
                ...
                <tbody>
                  {fakeBookings.map(b => (
                    <tr key={b.id} className="table-row">
                      ...
                      <td style={{ padding: "14px 16px", color: "#888", whiteSpace: "nowrap", maxWidth: 180 }}>{b.from} → {b.to}</td>
                      <td style={{ padding: "14px 16px", whiteSpace: "nowrap" }}>{b.service}</td>
                      <td style={{ padding: "14px 16px", color: "#888", whiteSpace: "nowrap" }}>{b.driver}</td>
                      <td style={{ padding: "14px 16px", fontWeight: 600, whiteSpace: "nowrap" }}>{b.fare}</td>
                      <td style={{ padding: "14px 16px", whiteSpace: "nowrap" }}>
                        <span className="badge" style={{ background: `rgba(${b.status==="Completed"?"34,197,94":b.status==="Active"?"255,204,0":b.status==="Pending"?"100,100,100":"239,68,68"},0.15)`, color: statusColor(b.status) }}>{b.status}</span>
                      </td>
          ...
          {/* Drivers */}
          <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, overflow: "hidden", marginTop: 24 }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #2A2A2A", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid #2A2A2A" }}>
                    {["ID","Name","Phone","Service","Rating","Rides","Status","Actions"].map(h => (
                ...
                <tbody>
                  {fakeDrivers.map(d => (
                    <tr key={d.id} className="table-row">
                      <td style={{ padding: "14px 16px", color: "#555" }}>{d.id}</td>
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button style={{ background: "rgba(255,204,0,0.1)", border: "none", borderRadius: 8, padding: "6px 10px", color: "#FFCC00", cursor: "pointer", fontSize: 12 }}>Edit</button>
                          <button style={{ background: "rgba(239,68,68,0.1)", border: "none", borderRadius: 8, padding: "6px 10px", color: "#ef4444", cursor: "pointer", fontSize: 12 }}>Remove</button>
                        </div>
                      </td>
                    </tr>
                  ))}
  ...
  const DriverPanel = () => {
    const driverStats = [
      { label: "Today's Earnings", value: "₹1,240", icon: "wallet" },
      { label: "Rides Today", value: "8", icon: "car" },
      { label: "Rating", value: "4.8 ★", icon: "star" },
      { label: "Hours Online", value: "5h 20m", icon: "clock" },
    ];
    const rideRequests = [
      { id: "RR001", from: "Civil Lines", to: "Railway Station", service: "Cab", fare: "₹95", dist: "3.2 km" },
      { id: "RR002", from: "Bus Stand", to: "Nagar Palika", service: "Cab", fare: "₹60", dist: "2.1 km" },
    ];
    const rideHistory = [
      { id: "BR2026001", from: "Hospital Road", to: "Market", fare: "₹45", time: "10:30 AM", rating: "5 ★" },
      { id: "BR2026002", from: "College Road", to: "Bank", fare: "₹70", time: "9:00 AM", rating: "4 ★" },
      { id: "BR2026003", from: "Bus Stand", to: "Sector 3", fare: "₹55", time: "8:15 AM", rating: "5 ★" },
    ];
    return (
      <div style={{ paddingTop: 70, padding: "70px 24px 40px", maxWidth: 900, margin: "0 auto" }}>
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 16, marginBottom: 28 }}>
          {driverStats.map(s => (
            <div key={s.label} style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 20 }}>
              ...
              <div className="sora" style={{ fontSize: 24, fontWeight: 800, marginBottom: 4 }}>{s.value}</div>
        ...
        {driverOnline && (
          <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, padding: 24, marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              <h3 className="sora" style={{ fontSize: 16, fontWeight: 700 }}>Incoming Ride Requests</h3>
              <span className="badge badge-yellow animate-pulse">LIVE</span>
            </div>
            {rideRequests.map(r => (
              <div key={r.id} style={{ background: "#111", borderRadius: 12, padding: 20, marginBottom: 12, border: "1px solid #2A2A2A" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <div style={{ fontWeight: 600, marginBottom: 4 }}>{r.from} → {r.to}</div>
                    <div style={{ color: "#888", fontSize: 13 }}>{r.service} · {r.dist}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div className="sora" style={{ color: "#FFCC00", fontSize: 18, fontWeight: 800 }}>{r.fare}</div>
                  </div>
                </div>
        ...
        {/* Map placeholder */}
        <div style={{ background: "#111", border: "1px solid #1A1A1A", borderRadius: 16, height: 200, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24, position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 60%, rgba(255,204,0,0.06) 0%, transparent 70%)" }}/>
          <div style={{ textAlign: "center", color: "#555" }}>
            <Icon name="mapPin" size={32} style={{ color: "#FFCC00", margin: "0 auto 8px" }}/>
            <p style={{ fontSize: 13 }}>Live Navigation Map — Ballia, UP</p>
            <p style={{ fontSize: 11, color: "#444", marginTop: 4 }}>Google Maps integration active</p>
          </div>
        </div>
        ...
        <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, overflow: "hidden" }}>
          <div style={{ padding: "20px 24px", borderBottom: "1px solid #2A2A2A" }}>
            <h3 className="sora" style={{ fontSize: 16, fontWeight: 700 }}>Today's Ride History</h3>
          </div>
          {rideHistory.map(r => (
            <div key={r.id} className="table-row" style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 500, marginBottom: 2 }}>{r.from} → {r.to}</div>
  ...
  const ProfilePage = () => (
    <div style={{ paddingTop: 90, padding: "90px 24px 40px", maxWidth: 700, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 32 }}>
        <div style={{ width: 72, height: 72, background: "#FFCC00", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#0A0A0A", fontFamily: "'Sora',sans-serif", fontSize: 28, fontWeight: 800 }}>
          {user?.name?.[0] || "U"}
        </div>
        <div>
          <h1 className="sora" style={{ fontSize: 26, fontWeight: 800 }}>{user?.name || "User"}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        {[["8","Total Rides"],["₹640","Wallet Balance"],["4.9★","Your Rating"],["2","Saved Places"]].map(([n,l]) => (
      ...
      <div style={{ background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, overflow: "hidden", marginBottom: 20 }}>
        ...
        {[
          ...
          <div key={i} className="table-row" style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontWeight: 500, marginBottom: 2, fontSize: 14 }}>{r.from} → {r.to}</div>
              <div style={{ color: "#555", fontSize: 12 }}>{r.date} · {r.service}</div>
            </div>
            <div style={{ fontWeight: 700, color: "#FFCC00" }}>{r.fare}</div>
          </div>
        ))}
      </div>
